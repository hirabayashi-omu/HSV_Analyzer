...
function updateLayout() {
    ...
    if (mode === 'Mobile') {
        ...
        // Auto: simple width check or UA check
        const isMobileUA = /Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        if (window.innerWidth <= 1024 || isMobileUA) {
            body.classList.add('is-mobile');
        } else {
            body.classList.remove('is-mobile');
        }

        // Update button state based on new layout
        if (typeof updateMenuButton === 'function') updateMenuButton();
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function toggleSidebar(forceClose = false) {
            ...
            if (isMobile) {
                // Mobile: Default Hidden (left: -250). Open adds class .open (left: 0).
                ...
                // PC: Default Visible. Collapsing adds class .closed (margin-left: -320).
                const isClosed = sidebar.classList.contains('closed');
                // Check if currently open (not closed)
